...
The 3.3V output from FTDI FT232R chips/adapters or Arduino boards _do not_ supply sufficient current to power an ESP8266 or ESP32 (it may seem to work sometimes, but it won't work reliably). Other USB TTL/serial adapters may also be marginal.
...
Running `esptool.py --trace` will dump all serial interactions to the standard output (this is _a lot_ of output). This can be helpful when debugging issues with the serial connection, or when providing information for bug reports.
